...
## 충돌 기록 #2
### 상황 (What happened)
1. 다른 브랜치에서 `conflict-resolution.md`파일이 `main` 브랜치에 merge
2. 현재 브랜치에서 `conflict-resolution.md`를 수정 중인 상황이라 `conflict` 발생
...
## 충돌 기록 #2
### 상황 (What happened)
- 충돌이 발생한 브랜치 및 파일 경로 기술

### 충돌 내용 (Conflict markers)
```
<<<<<<< HEAD
이곳에 HEAD(현재 브랜치) 기준 코드 작성
=======
이곳에 머지/리베이스 대상 브랜치의 코드 작성
>>>>>>> feature/<브랜치명>
```

### 해결 과정 (How)
- 선택한 해결 전략 (keep both / choose one / refactor) 및 그 이유
- 실제 해결을 위해 수행한 명령어 또는 절차 기술

### 결과 (Outcome)
- 최종 병합 결과 요약 및 관련 PR/커밋 링크

### 배운 점 (Learnings)
- 향후 유사한 충돌을 예방하기 위한 방안
=======
## 충돌 기록 #1
...
* 향후 예방 대책: 공통 문서 작업을 진행할 때는 파일의 시작 지점(1번 줄)부터 무작정 쓰기보다, 팀원 간 사전에 영역을 분할하거나 목차를 먼저 고정해 두어 동일한 라인이 겹쳐 발생하는 비자명 충돌을 최소화하고자 함.
>>>>>>> 427133081e57bcdd5bdbc019050d2402fb2060df
````

### 해결 과정 (How)
- 기존에 작성되던 내용과 merge된 내용이 둘 다 유효하고 순서만 조정하면 되는 상황이라 `keep both` 전략을 사용

### 결과 (Outcome)
- main에 merge된 내용을 상단에 위치하고 이후에 작성 중이던 내용이 오도록 수정

### 배운 점 (Learnings)
* 마크다운 문서에 충돌이 있어서 해당 내용을 보고서를 작성하는 과정에서 ```로 감싸진 내용이 깨지는 문제가 발생
* 가장 바깥에 있는 백틱 갯수를 더 많이 하면 내부에 있는 백틱으로 감싸진 내용이 깨지지 않는 것을 알게 됐습니다.